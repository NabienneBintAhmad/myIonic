...
export class TransactionService {
  ...
  private endpoint2 = 'http://127.0.0.1:8000/api/infotarif';
  private endpoint3 = 'http://127.0.0.1:8000/api/inforetrait';
  ...
  retrait(formData) {
    ...
    envoie(formData) {
      ...
      formData2.append('envoyeurCni', formData.envoyeurCni);
      console.log(formData);
   ...
   infotarif(tarif):Observable<any> {
  
        return this.httpClient.post<any>(this.endpoint2,tarif);
      }

      inforetraitt(inforetrait):Observable<any> {
      return this.httpClient.post<any>(this.endpoint3,inforetrait);
